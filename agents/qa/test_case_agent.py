    ...
    def create_test_cases(self, 
        ...
        try:
            ...
            if not all_test_cases:
                # Skip test case creation if no LLM-generated cases (no fallback per user requirement)
                self.logger.warning(f"No test cases generated by LLM for {user_story_title}, skipping test case creation")
                return {
                    'success': False,
                    'error': 'Failed to generate test cases from LLM - skipping to avoid generic content'
                }
...
"""
    ...
    def _generate_test_cases_by_category(self, 
        ...
        try:
            ...
            # Call LLM to generate test cases using template system
            # The run() method will internally call get_prompt(template_context)
            user_input = f"Create {category} test cases for the user story: {user_story.get('title', 'Unknown Story')}"
            response = self.run(user_input, template_context)
...
Focus on timing boundaries, input limits, and edge cases. Generate 3-5 comprehensive boundary test cases."""
    ...
    def _generate_fast_boundary_cases(self, 
        ...
        try:
            ...
            # Call LLM to generate boundary test cases using template system
            # The run() method will internally call get_prompt(template_context)
            user_input = f"Create comprehensive boundary test cases for the user story: {user_story.get('title', 'Unknown Story')}"
            response = self.run(user_input, template_context)
